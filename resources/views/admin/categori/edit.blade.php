...
@section('content')
    <div class="card">
        <div class="card-header">
            <a href="{{ route('product.categori.index') }}" class="btn btn-primary btn-5 d-none d-sm-inline-block" data-bs-toggle="modal" data-bs-target="#modal-report">
                Kembali
            </a>
        </div>
        <form action="{{ route('product.categori.update', ['id' => $categori->id]) }}" method="POST">
            @csrf
            @method("PUT")
            <div class="card-body">
