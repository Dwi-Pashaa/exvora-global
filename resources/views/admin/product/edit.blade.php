...
@section('content')
    <form action="{{ route('product.list.update', ['id' => $product->id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method("PUT")
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="form-group mb-3">
                    ...
                    <select name="categori_id" id="categori_id" class="form-control @error('categori_id') is-invalid @enderror">
                        ...
                        @foreach ($categorie as $item)
                            <option value="{{$item->id}}" {{ $product->categori_id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                        @endforeach
                    </select>
                    @error('categori_id')
                        <span class="invalid-feedback">
                ...
                <div class="form-group mb-3">
                    ...
                    <select name="sub_categori_id" id="sub_categori_id" class="form-control @error('sub_categori_id') is-invalid @enderror">
                        <option value="{{ $product->sub_categori_id }}">{{ $product->subCategori->name }}</option>
                    </select>
                ...
                <div class="form-group mb-3">
                    ...
                    <textarea name="desc_id" id="tinymce-id" class="form-control @error('desc_id') is-invalid @enderror">{{ $product->desc_id }}</textarea>
                    @error('desc_id')
                        <span class="invalid-feedback">
                            {{ $message }}
                        </span>
                ...
                <div class="form-group mb-3">
                    ...
                    <textarea name="desc_en" id="tinymce-en" class="form-control @error('desc_en') is-invalid @enderror">{{ $product->desc_en }}</textarea>
                    @error('desc_en')
                        <span class="invalid-feedback">
                            {{ $message }}
                        </span>
                ...
                <div class="form-group mb-3">
                    ...
                    <select name="sort" id="sort" class="form-control @error('sort') is-invalid @enderror">
                        <option value="">Pilih</option>
                        @for ($i = 1; $i <= 100; $i++)
                            <option value="{{$i}}" {{ $product->sort == $i ? 'selected' : '' }}>{{$i}}</option>
                        @endfor
                    </select>
                    @error('sort')
                        <span class="invalid-feedback">
